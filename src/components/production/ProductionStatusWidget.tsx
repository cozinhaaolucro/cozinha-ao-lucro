...
const ProductionStatusWidget = () => {
    ...
    const [progressData, setProgressData] = useState<{ id: string, progress: number, isOverdue: boolean }[]>([]);
    ...
    useEffect(() => {
        ...
        fetchData();
        const interval = setInterval(fetchData, 5000); // Poll more frequently for live updates
    ...
    useEffect(() => {
        ...
        const updateProgress = () => {
            ...
            const newProgress = activeOrders.map(order => {
                if (!order.production_started_at) return { id: order.id, progress: 0, isOverdue: false };
                ...
                const isOverdue = elapsedMinutes > idealTime && idealTimeCallback > 0; // Only mark overdue if ideal time was actually set

                return { id: order.id, progress: progressPercent, isOverdue };
            });
    ...
    return (
        <div className="fixed top-4 left-1/2 -translate-x-1/2 z-50 md:left-auto md:top-auto md:bottom-6 md:right-24 md:translate-x-0">
            <motion.div
                ...
                animate={{
                    width: isHovered ? 280 : (activeOrders.length > 0 ? 80 : 60),
                    height: isHovered ? 'auto' : (activeOrders.length > 0 ? 80 : 60),
                    ...
                    {!isHovered && (
                        <div className="flex flex-col items-center justify-center w-full h-full gap-1">
                            {activeOrders.length > 0 ? (
                                <>
                                    <ChefHat className={`w-6 h-6 ${progressData.some(p => p.isOverdue) ? 'text-red-400 animate-bounce' : 'text-blue-400'}`} />
                                        ...
                                        return (
                                            <div key={order.id} className="space-y-1">
                                                <div className="flex justify-between text-xs">
                                                    <span className="text-white/80">#{order.id.slice(0, 4)} - {order.customer?.name?.split(' ')[0] || 'Balcão'}</span>
